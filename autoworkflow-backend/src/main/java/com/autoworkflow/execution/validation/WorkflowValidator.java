...
public class WorkflowValidator {
    ...
    @Autowired
    public WorkflowValidator(NodeStrategyRegistry registry, ConditionEvaluator conditionEvaluator) {
        this.registry = registry;
        this.conditionEvaluator = conditionEvaluator;
    }
    public WorkflowValidator(NodeStrategyRegistry registry) { this(registry, new ConditionEvaluator()); }
    ...
    private WorkflowValidationResult validateInternal(JsonNode canvasNodes, JsonNode canvasEdges, boolean requireTrigger) {
        ...
        Set<String> ids = new HashSet<>(); Map<String, JsonNode> byId = new HashMap<>(); Map<String, String> types = new HashMap<>();
        ...
        for (JsonNode node : canvasNodes) {
            ...
            byId.put(id, node); types.put(id, type); if (registry.isTriggerType(type)) hasTrigger = true;
        ...
        Set<String> edgeKeys = new HashSet<>(); Map<String, List<String>> graph = new HashMap<>(); Map<String, List<JsonNode>> outgoing = new HashMap<>(); Map<String, Integer> incoming = new HashMap<>();
    ...
    private WorkflowValidationResult validateAdvancedControlFlow(Map<String, JsonNode> byId, Map<String, String> types, Map<String, Integer> incoming, Map<String, List<JsonNode>> outgoing) {
            ...
            if ("loop".equals(type)) {
                ...
                while (!q.isEmpty()) {
                    ...
                    for (JsonNode edge : outgoing.getOrDefault(current, List.of())) {
                        ...
                        } else if (target.equals(id)) return WorkflowValidationResult.invalid("Loop body cannot point back to Loop node '" + id + "'.");
                        else q.addLast(target);
                    }
                }
    ...
    private boolean dfs(String curr, Map<String, List<String>> graph, Set<String> visited, Set<String> stack, List<String> path) {
        if (stack.contains(curr)) { path.add(curr); return true; } if (visited.contains(curr)) return false;
