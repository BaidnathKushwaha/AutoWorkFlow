...
public class ConditionEvaluator {
    ...
    private static final Set<String> OPERATORS = Set.of("equals", "not_equals", "contains", "not_contains", "starts_with", "ends_with", "greater_than", "greater_than_or_equal", "less_than", "less_than_or_equal", "is_empty", "is_not_empty", "exists", "not_exists", "is_true", "is_false");
    private static final Set<String> VALUE_FREE = Set.of("exists", "not_exists", "is_true", "is_false", "is_empty", "is_not_empty");
    ...
    public JsonNode normalize(JsonNode condition) {
        ...
        if (condition.isTextual()) {
            if (condition.asText().isBlank()) throw config("Condition JSON cannot be empty.");
            try { return JsonUtils.mapper().readTree(condition.asText()); } catch (Exception e) { throw config("Condition must contain valid JSON."); }
        }
        return condition;
    }
    ...
    private boolean evaluateNode(JsonNode input, JsonNode node, int depth) {
        ...
        if (conditions != null) {
            ...
            if (logic.equals("AND")) { for (JsonNode child : conditions) if (!evaluateNode(input, child, depth + 1)) return false; return true; }
        ...
        return switch (operator) {
            ...
            case "starts_with" -> stringOp(actual, expected, true);
            case "ends_with" -> stringOp(actual, expected, false);
    ...
    private void validateNode(JsonNode node, int depth) {
        ...
        String operator = node.path("operator").asText("").trim();
        if (!OPERATORS.contains(operator)) throw config("Unsupported condition operator: '" + operator + "'.");
        if (!VALUE_FREE.contains(operator) && !node.has("value")) throw config("Condition operator '" + operator + "' requires a value.");
    }
    ...
    public JsonNode resolvePath(JsonNode root, String path) {
        ...
        for (String segment : path.split("\\.")) {
            ...
            if (current.isArray()) {
                ...
                try { current = current.path(Integer.parseInt(segment)); } catch (NumberFormatException e) { return com.fasterxml.jackson.databind.node.MissingNode.getInstance(); }
            } else if (current.isObject()) current = current.path(segment);
            else return com.fasterxml.jackson.databind.node.MissingNode.getInstance();
        }
        return current;
    }
    ...
    private boolean contains(JsonNode actual, JsonNode expected) {
        ...
        if (actual.isArray()) { for (JsonNode item : actual) if (expected != null && equals(item, expected)) return true; return false; }
        throw incompatible("contains", actual, expected);
    }
    private boolean stringOp(JsonNode actual, JsonNode expected, boolean starts) {
        if (!actual.isTextual() || expected == null || !expected.isTextual()) throw incompatible(starts ? "starts_with" : "ends_with", actual, expected);
        return starts ? actual.textValue().startsWith(expected.textValue()) : actual.textValue().endsWith(expected.textValue());
    }
    private int compare(JsonNode actual, JsonNode expected) {
        if (!actual.isNumber() || expected == null || !expected.isNumber()) throw incompatible("numeric comparison", actual, expected);
        return actual.decimalValue().compareTo(expected.decimalValue());
    }
    private boolean isEmpty(JsonNode node) { return node == null || node.isMissingNode() || node.isNull() || (node.isTextual() && node.textValue().isEmpty()) || (node.isArray() && node.isEmpty()) || (node.isObject() && node.isEmpty()); }
    private boolean requireBoolean(JsonNode node, String op) { if (node == null || node.isMissingNode() || !node.isBoolean()) throw incompatible(op, node, null); return node.booleanValue(); }
    private IntegrationException incompatible(String op, JsonNode actual, JsonNode expected) { return config("Condition operator '" + op + "' received incompatible types: actual=" + typeOf(actual) + ", expected=" + typeOf(expected) + "."); }
    private String typeOf(JsonNode n) { if (n == null || n.isMissingNode()) return "missing"; if (n.isNull()) return "null"; if (n.isTextual()) return "text"; if (n.isNumber()) return "number"; if (n.isBoolean()) return "boolean"; if (n.isArray()) return "array"; if (n.isObject()) return "object"; return n.getNodeType().name().toLowerCase(Locale.ROOT); }
