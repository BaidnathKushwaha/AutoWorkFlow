...
public class TransformStrategy implements NodeStrategy {
    ...
    public NodeExecutionResult execute(NodeExecutionContext ctx) {
        ...
        if (mappings.isArray() && !mappings.isEmpty()) {
            for (JsonNode row : mappings) {
                ...
                if (target.isEmpty() || source.isEmpty()) {
                    return NodeExecutionResult.failed("Transform mapping requires non-empty output and source paths.");
                }
                JsonNode value = conditionEvaluator.resolvePath(input, source);
                String strip = row.path("strip").asText("");
                if (!strip.isEmpty() && value.isTextual() && value.asText().startsWith(strip)) {
                    value = JsonUtils.mapper().getNodeFactory().textNode(value.asText().substring(strip.length()));
                }
            ...
            while (it.hasNext()) {
                ...
                if (source.isEmpty()) {
                    return NodeExecutionResult.failed("Transform mapping for '" + e.getKey() + "' has an empty source path.");
                }
        ...
        if (conversions != null && conversions.isObject()) {
            ...
            while (it.hasNext()) {
                ...
                if (converted == null) {
                    return NodeExecutionResult.failed("Transform cannot convert field '" + e.getKey() + "' to '" + e.getValue().asText() + "'.");
                }
        ...
        if (filter != null && filter.isObject()) {
            ...
            if (!array.isArray() || condition == null) {
                return NodeExecutionResult.failed("Transform filter requires arrayPath and condition.");
            }
            conditionEvaluator.validate(condition);
            ArrayNode kept = JsonUtils.mapper().createArrayNode();
            for (JsonNode item : array) {
                if (conditionEvaluator.evaluate(item, condition)) kept.add(item.deepCopy());
            }
        ...
        if (map != null && map.isObject()) {
            ...
            if (!array.isArray() || fields == null || !fields.isObject()) {
                return NodeExecutionResult.failed("Transform map requires arrayPath and a fields object.");
            }
            ArrayNode mapped = JsonUtils.mapper().createArrayNode();
            for (JsonNode item : array) {
                ObjectNode row = JsonUtils.mapper().createObjectNode();
                fields.fields().forEachRemaining(e -> row.set(
                        e.getKey(),
                        normalizeMissing(conditionEvaluator.resolvePath(item, e.getValue().asText()))
                ));
                mapped.add(row);
            }
    ...
    private JsonNode normalizeMissing(JsonNode value) {
    ...
    private JsonNode parseJson(JsonNode value, String name) {
        if (value == null || value.isMissingNode() || value.isNull() || (value.isTextual() && value.asText().isBlank())) return null;
        if (value.isObject() || value.isArray()) return value;
        try {
            return JsonUtils.mapper().readTree(value.asText());
        } catch (Exception e) {
            throw new IllegalArgumentException("Transform " + name + " must contain valid JSON.");
        }
    }
    ...
    private JsonNode convert(JsonNode value, String type) {
        ...
        switch (normalized) {
            ...
            case "bool":
                ...
                if (value.isTextual() && (value.asText().equalsIgnoreCase("true") || value.asText().equalsIgnoreCase("false"))) {
                    return JsonUtils.mapper().getNodeFactory().booleanNode(Boolean.parseBoolean(value.asText()));
                }
                return null;
            default:
                return null;
