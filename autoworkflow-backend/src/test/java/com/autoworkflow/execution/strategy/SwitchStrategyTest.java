...
class SwitchStrategyTest {
    ...
    private NodeExecutionContext ctx(JsonNode config, JsonNode input) {
        return new NodeExecutionContext(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), "node-1", "switch", config, input);
    }
    ...
    @Test void noMatchingCase_fallsBackToDefaultCase() throws Exception {
    ...
    @Test void missingFieldInPayload_fallsBackToDefaultCase() throws Exception {
    ...
    @Test void missingFieldInPayload_noDefaultCase_failsClearly() throws Exception {
        JsonNode input = JsonUtils.mapper().readTree("{\"somethingElse\":\"x\"}");
        NodeExecutionResult result = strategy.execute(ctx(config("match", null, "Strong", "Moderate", "Weak"), input));
        assertThat(result.success()).isFalse();
        assertThat(result.error()).contains("missing/null");
    }

    @Test void noMatchingCase_andNoDefaultCase_failsClearly() throws Exception {
        JsonNode input = JsonUtils.mapper().readTree("{\"match\":\"Unknown\"}");
        NodeExecutionResult result = strategy.execute(ctx(config("match", null, "Strong", "Moderate", "Weak"), input));
        assertThat(result.success()).isFalse();
    ...
    @Test void missingFieldConfig_failsClearly() throws Exception {
        ObjectNode cfg = JsonUtils.mapper().createObjectNode();
        cfg.putArray("cases").add("Strong");
        NodeExecutionResult result = strategy.execute(ctx(cfg, JsonUtils.mapper().createObjectNode()));
        assertThat(result.success()).isFalse();
        assertThat(result.error()).contains("field");
    }

    @Test void missingCasesConfig_failsClearly() throws Exception {
        ObjectNode cfg = JsonUtils.mapper().createObjectNode();
        cfg.put("field", "match");
        NodeExecutionResult result = strategy.execute(ctx(cfg, JsonUtils.mapper().createObjectNode()));
        assertThat(result.success()).isFalse();
    ...
    @Test void casesConfigured_butAllBlank_failsClearly() throws Exception {
        ObjectNode cfg = JsonUtils.mapper().createObjectNode();
        cfg.put("field", "match");
        cfg.putArray("cases").add("  ").add("").add("   ");
        NodeExecutionResult result = strategy.execute(ctx(cfg, JsonUtils.mapper().createObjectNode()));
        assertThat(result.success()).isFalse();
        assertThat(result.error()).contains("at least one");
    }

    @Test void duplicateCasesInConfig_matchSafely_returnsSingleBranchKey() throws Exception {
        ObjectNode cfg = JsonUtils.mapper().createObjectNode();
        cfg.put("field", "match");
        cfg.putArray("cases").add("Strong").add("Strong").add("Weak");
        NodeExecutionResult result = strategy.execute(ctx(cfg, JsonUtils.mapper().readTree("{\"match\":\"Strong\"}")));
        assertThat(result.success()).isTrue();
        assertThat(result.branchKey()).isEqualTo("Strong");
    }
}
